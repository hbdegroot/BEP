...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun  6 20:40:21 2023
...
q = pd.read_csv('save_dir/data/20230606_049.csv', delimiter=';', decimal=',', keep_default_na = False)
...
d = pd.read_csv('save_dir/data/20230606_052.csv', delimiter=';', decimal=',', keep_default_na = False)
...
def func(arr, period = 24*6, offset=0):
    arr = replacer(arr[:,3])
    return np.append(np.zeros(offset),moving_average(arr,period)[offset::period])
...
ax.set_xlim(0,365)
ax.set_xlabel('day from 01-01-2021')
ax.set_ylabel('$Q$')
ax.legend()
