...
# -*- coding: utf-8 -*-
"""
Created on Sat Jun 10 11:38:43 2023
    ...
    def solf(h):
        ...
        for j in range(len(tau)):
            if abs(Stot9 - integrate.trapz(sol[j]*pi*(rho+q), rho)) < perc*abs(Stot9 - Stot0) and dstot ==0:
                dstot = tau[j]
        return dstot, dt_min, dt_max
    
    dstot1, dt_min1, dt_max1 = solf(h1)
    dstot2, dt_min2, dt_max2 = solf(h2)
                
    return dstot1, dstot2, dt_min1, dt_min2, dt_max1, dt_max2, -np.log(perc)/labda[0]


q = 0.02
P = np.linspace(0.01,0.5,4)
# mspace = np.linspace(1.1,10,4)
# ds1, ds2, tl0 = [np.zeros(len(P)) for i in range(len(mplus))], [np.zeros(len(P)) for i in range(len(mplus))], [np.zeros(len(P)) for i in range(len(mplus))]

# for j in range(len(mplus)):
#     print('Calculation', j, '/', len(mplus), 'started')
#     for i in range(len(P)):
#         ds1[j][i], ds2[j][i], tl0[j][i] = dmlts(P[i],q, mplus[j])


# fig, ax = plt.subplots(1,1, figsize=(8,8))
# for j in range(len(mplus)):
#     ax.plot(P, ds1[j], 'C'+str(j)+'.', label='m = ' + str(mplus[j]))
#     ax.plot(P, ds2[j], 'C'+str(j)+'*', label='m = ' + str(mplus[j]))
...
for j in range(len(mspace)):
    ...
    for i in range(len(P)):
        ds1[j][i], ds2[j][i], dtmin1[j][i], dtmin2[j][i], dtmax1[j][i], dtmax2[j][i], evts[j][i] = dmlts(P[i], q, mspace[j])
