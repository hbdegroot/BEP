...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun  5 12:10:15 2023
...
def dmlts(Pe, multiplier=2):
    ...
    return dstot1, dstot2, T_labda[0], T_labda[1]


Pe = np.linspace(0.1,10,10)
mplus = np.linspace(1.1,10,4)
mspace = np.append(mplus, 1/mplus)
ds1, ds2, tl0, tl1 = [np.zeros(len(Pe)) for i in range(len(mspace))], [np.zeros(len(Pe)) for i in range(len(mspace))], [np.zeros(len(Pe)) for i in range(len(mspace))], [np.zeros(len(Pe)) for i in range(len(mspace))]

for j in range(len(mspace)):
    print('Calculation', j, '/', len(mspace), 'started')
    for i in range(len(Pe)):
        ds1[j][i], ds2[j][i], tl0[j][i], tl1[j][i] = dmlts(Pe[i],mspace[j])


fig, ax = plt.subplots(1,1, figsize=(8,8))
for j in range(len(mspace)//2):
    ax.plot(Pe, ds1[j], 'C'+str(j), label='salt (down)' + str(j))
    ax.plot(Pe, ds2[j], 'C'+str(j), label='salt (up)' + str(j))
...
for i in range(0,Np,Np//5):
    plt.plot(mspace, [ds1[j][i] for j in range(Nm)], '.', color='C'+str(c), label='Pe = ' + str(round(Pe[i],3)))
...
plt.figure()
c=0
for j in range(0,Nm//5):
    plt.plot(Pe,ds1[:][j], '.', color = 'C' +str(c), label='m = ' +str(round(mspace[j],3)))
