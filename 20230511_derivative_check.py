...
# -*- coding: utf-8 -*-
"""
Created on Thu May 11 09:09:43 2023
...
# Guess
time_constant = min(labda_r_n(1), labda_x_n(1))/5
...
def sol_r(r,t):
    temp = T_r(t)
    res = temp @ np.array([phi_r_n(r,n) for n in range(1,N_r)])
    der = (f_interp(t)-1)/(a-R) + temp @ np.array([phi_r_prime(a,n) for n in range(1,N_r)])
    return res + psi(r,t) , der

def sol_x(x,t):
    temp = T_x(t)
    res = temp @ np.array([phi_x_n(x,n) for n in range(1,N_x)])
    der = -f_interp(t)/L + temp @ np.array([phi_x_prime(0,n) for n in range(1,N_x)])
    return res + xi(x,t) , der
...
t = np.linspace(0,1e5,30)

result = [np.zeros(10) for i in range(len(t))]
sx_ser = [0 for i in range(len(t))]
sr_ser = [0 for i in range(len(t))]

for i in range(len(t)):
    T = len(t)//10
    if i % T == 0:
        print('time steps @ ' + str(i//T*10) + '%')
    solx, derx = sol_x(x,t[i])
    solr, derr = sol_r(r,t[i])
    result[i] = np.append(solx[::-1], solr)
    sx_ser[i] = derx
    sr_ser[i] = derr
